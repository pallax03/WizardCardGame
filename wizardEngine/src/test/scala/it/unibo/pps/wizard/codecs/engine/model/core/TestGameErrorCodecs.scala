...
class TestGameErrorCodecs extends AnyWordSpec with Matchers:
  ...
  "GameErrorCodecs" should:
    ...
    "encode and decode GameError.CardNotAllowed (CardNotInHand) correctly" in:
      import cards.Card.*
      val error: GameError = GameError.CardNotAllowed(CardNotAllowedReasons.CardNotInHand(List(Ten of Blue)))
      val jsonString = error.asJson.noSpaces
    ...
    "encode and decode GameError.CardNotAllowed (MustFollowColor) correctly" in:
      import cards.Card.*
      val error: GameError = GameError.CardNotAllowed(CardNotAllowedReasons.MustFollowColor(Red, List(Ten of Blue)))
      val jsonString = error.asJson.noSpaces
    ...
    "encode and decode GameError.InconsistentState (HandNotFoundFor) correctly" in:
      val error: GameError = GameError.InconsistentState(InconsistentStateReasons.HandNotFoundFor(PlayerId(1)))
      val jsonString = error.asJson.noSpaces
