...
class TestLobbyCodecs extends AnyWordSpec with Matchers:
  ...
  "LobbyCodecs" should:
    ...
    val lobby = Lobby(LobbyId("uuid-1234"), players, LobbyStatus.WAITING, GameConfiguration())
    "encode and decode Lobby correctly" in:
      val jsonString = lobby.toJson
      jsonString shouldBe """{"lobbyId":"uuid-1234","players":[{"id":1,"name":"Alice","difficulty":null,"isOnline":false,"secret":null},{"id":2,"name":"Bot-1","difficulty":"Dumb","isOnline":false,"secret":null}],"status":"WAITING","configuration":{"timer":60,"gracePeriodSeconds":3,"maxStrikes":2}}"""
