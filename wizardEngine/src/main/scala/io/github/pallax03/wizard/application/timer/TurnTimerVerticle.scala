...
class TurnTimerVerticle(
  ...
  private def handleTurnEvent(jsonStr: String): Unit =
    (for
      ...
      strikesResp <- redisClient.send(Request.cmd(Command.GET).arg(ChannelsKeys.afkStrikes(payload.lobbyId, payload.playerId))).asScala
      strikes = Option(strikesResp).map(_.toString.toInt).getOrElse(0)
      ttl = Math.max(1, (lobby.configuration.timer + GameConfiguration.gracePeriodSeconds) / Math.pow(2, strikes).toInt)
      _ <- redisClient.send(Request.cmd(Command.SET).arg(ChannelsKeys.turnTimer(payload.lobbyId, payload.playerId)).arg("1").arg("EX").arg(ttl.toString)).asScala
    yield ()).recover(_ => ())
        ...
        lobbyStatePort
          ...
          .flatMap:
            case Right((offlineIds, newStatus)) if offlineIds.nonEmpty =>
              for
                _ <- Future.sequence(offlineIds.map(pid => pubSubPort.publish(ChannelsKeys.pubSubLobbyChannel(lobbyId), SystemEvent.afkReplaced(pid).toJson)))
                _ <- if newStatus == LobbyStatus.IN_GAME then inboundPort.resumeGame(lobbyId) else Future.unit
              yield ()
            case _ => Future.unit
          .recover: ex =>
            pubSubPort.publish(ChannelsKeys.LOGS_CHANNEL, s"ERROR:[DisconnectTimer] Failed for $lobbyIdStr: ${ex.getMessage}")
