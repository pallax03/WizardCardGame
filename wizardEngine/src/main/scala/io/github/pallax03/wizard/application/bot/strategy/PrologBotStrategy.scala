...
class PrologBotStrategy(port: AIPort) extends BotStrategy:
  ...
  override protected def chooseCard(l: LobbyId, e: InvitationEvent.WaitingForCard): Future[Card] = unwrap(port.bestCard(l, e.destinationId))
  override protected def chooseBid(l: LobbyId, e: InvitationEvent.WaitingForBid): Future[Bid] = unwrap(port.placeBid(l, e.destinationId))
  override protected def chooseTrump(l: LobbyId, e: InvitationEvent.WaitingForTrump): Future[Card.Color] = unwrap(port.resolvedTrumpColor(l, e.destinationId))
