...
class BotManagerVerticle(
  ...
  override def start(): Unit =
    redisClient
      .send(Request.cmd(Command.XGROUP).arg("CREATE").arg(ChannelsKeys.BOT_TASKS_STREAM).arg(ChannelsKeys.BOT_CONSUMER_GROUP).arg("$").arg("MKSTREAM"))
      .asScala.void.recover { case _ => () }
      .onComplete:
        case Success(_) =>
  ...
  private def poll(): Unit =
    redisClient.send(Request.cmd(Command.XREADGROUP).arg("GROUP").arg(ChannelsKeys.BOT_CONSUMER_GROUP).arg(consumerId).arg("COUNT").arg(BATCH_SIZE.toString).arg("STREAMS").arg(ChannelsKeys.BOT_TASKS_STREAM).arg(">"))
      .asScala.foreach(resp => if resp != null then dispatch(resp))

  private def reclaim(): Unit =
    redisClient.send(Request.cmd(Command.XAUTOCLAIM).arg(ChannelsKeys.BOT_TASKS_STREAM).arg(ChannelsKeys.BOT_CONSUMER_GROUP).arg(consumerId).arg(CLAIM_IDLE_MS.toString).arg("0-0").arg("COUNT").arg(BATCH_SIZE.toString))
      .asScala.foreach(resp => if resp != null && resp.get(1) != null then resp.get(1).asScala.foreach(processEntry))

  private def dispatch(resp: Response): Unit =
    val entries = try resp.get(ChannelsKeys.BOT_TASKS_STREAM) catch case _: Exception => Option(resp.get(0)).map(_.get(1)).orNull
  ...
  private def processEntry(entry: Response): Unit =
    try
      ...
      val dataJson = try fields.get(1).toString catch case _: Exception => fields.get("data").toString
          ...
          case Success(Right(lobby)) =>
            lobby.players.find(_.id == inv.destinationId).flatMap(_.difficulty) match
              case Some(diff) =>
                ...
                strat.resolveInvitation(task.lobbyId, inv).onComplete:
                  case Success(action) => vertx.setTimer(DEFAULT_BOT_DELAY_MS, _ => submitAndAck(task.lobbyId, inv, action, entryId))
                  case Failure(e)      => log(s"ERROR: Strategy failed: ${e.getMessage}"); ackEntry(entryId)
              case None => ackEntry(entryId)
          case _ => ackEntry(entryId)
      case _ => ackEntry(entryId)

  private def submitAndAck(lobbyId: LobbyId, inv: InvitationEvent, action: GameAction, entryId: String): Unit =
  ...
  private def ackEntry(entryId: String): Unit =
    redisClient.send(Request.cmd(Command.XACK).arg(ChannelsKeys.BOT_TASKS_STREAM).arg(ChannelsKeys.BOT_CONSUMER_GROUP).arg(entryId)).onComplete(_ => ())
