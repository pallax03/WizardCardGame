...
trait BotStrategy:
  ...
  protected def chooseTrump(lobbyId: LobbyId, event: InvitationEvent.WaitingForTrump): Future[Card.Color]

  final def resolveInvitation(lobbyId: LobbyId, invitation: InvitationEvent)(using ExecutionContext): Future[GameAction] =
    val actionFuture = invitation match
      case e: InvitationEvent.WaitingForCard  => chooseCard(lobbyId, e).map(GameAction.PlayCard(e.destinationId, _))
      case e: InvitationEvent.WaitingForBid   => chooseBid(lobbyId, e).map(GameAction.PlaceBid(e.destinationId, _))
      case e: InvitationEvent.WaitingForTrump => chooseTrump(lobbyId, e).map(GameAction.ResolveTrumpColor(e.destinationId, _))
