...
import io.circe._
import it.unibo.pps.wizard.engine.model.lobby.{BotsDifficulty, Lobby, LobbyPlayer, LobbyStatus}
...
object LobbyCodecs:
  ...
  given Decoder[LobbyStatus] = Decoder.decodeString.emapTry(s => scala.util.Try(LobbyStatus.valueOf(s)))

  given Encoder[BotsDifficulty] = Encoder.encodeString.contramap(_.toString)
  given Decoder[BotsDifficulty] = Decoder.decodeString.emapTry(s => scala.util.Try(BotsDifficulty.valueOf(s)))
